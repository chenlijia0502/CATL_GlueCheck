...
{
	std::string szResult;
	int nSearchStatus = KxXmlFun::SearchXmlGetValue(filePath, "检查设置", "极片亮度", szResult);
	if (!nSearchStatus)
	{
		return false;
	}
...
int CKxPlateEdgeCheck::CheckWhiteLine(const kxCImageBuf& SrcImg, KxCallStatus& hCall)
{
	...
	//增加一个白线位置缺陷用于判废
	{
		...
		//m_hResult.mapFeaturelists[nIndx]["标志位"] = (std::max)(0, nIndex - m_hResult.nEdgeStart)*1.0f;
		m_hResult.mapFeaturelists[nIndx][8] = std::make_pair("标志位", (std::max)(f1, f2) / (std::min)(f1, f2));
	...
	for (int i = 0; i < m_WhiteImg.nWidth; i++)
	{
		...
		if (bFind && pProject[i] > 0)
		{
			...
			int nIndx = (std::min)(m_hResult.nDefectCount++, (int)_MAX_DEFECT_COUNT - 1);
			m_hResult.mapFeaturelists[nIndx][0] = std::make_pair("点数", nLen*_WHITELINE_HEIGHT*1.0f);
			m_hResult.mapFeaturelists[nIndx][1] = std::make_pair("能量", 10000.0f);
			m_hResult.mapFeaturelists[nIndx][2] = std::make_pair("X坐标", (std::max)(0, nFindStart - 30) * 1.0f);
			m_hResult.mapFeaturelists[nIndx][3] = std::make_pair("Y坐标", (std::max)(0, m_nWhiteLineStart - 50) * 1.0f);
			m_hResult.mapFeaturelists[nIndx][4] = std::make_pair("缺陷宽", (std::min)(SrcImg.nWidth - 1, nLen + 60) *1.0f);
			m_hResult.mapFeaturelists[nIndx][5] = std::make_pair("缺陷高", (std::min)(SrcImg.nHeight - 1, rcCopy.Height() + 100) *1.0f);
			m_hResult.mapFeaturelists[nIndx][6] = std::make_pair("位置", _MID *1.0f);
...
int CKxPlateEdgeCheck::Check(const kxCImageBuf& SrcImg, kxCImageBuf& DstImg, KxCallStatus& hCall)
{
	...
	////-------------------------------------------------------------------------//
	////负极测试模块，为了测试程序崩溃在处理线程还是调焦线程，把处理线程屏蔽了
	////if (1)
	////{
	////	int nIndx = (std::min)(m_hResult.nDefectCount++, (int)_MAX_DEFECT_COUNT - 1);
	...
	////	m_hResult.mapFeaturelists[nIndx][4] = std::make_pair("缺陷宽", (m_CropImg.nWidth - 1)*1.0f);
	////	m_hResult.mapFeaturelists[nIndx][5] = std::make_pair("缺陷高", (m_CropImg.nHeight - 1)*1.0f);
	////	m_hResult.mapFeaturelists[nIndx][6] = std::make_pair("位置", -1.0f);
	...
	//	m_hResult.mapFeaturelists[nIndx][4] = std::make_pair("缺陷宽", (m_CropImg.nWidth - 1)*1.0f);
	//	m_hResult.mapFeaturelists[nIndx][5] = std::make_pair("缺陷高", (m_CropImg.nHeight - 1)*1.0f);
	//	//m_hResult.mapFeaturelists[nIndx][6] = std::make_pair("中心占比率", -1.0f);
	...
	//{
	//	int nIndx = (std::min)(m_hResult.nDefectCount++, (int)_MAX_DEFECT_COUNT - 1);
	//	//暂时拿第八位作为标志位(0是特指断的区域，1是线上的，2是其它区域，3是指极片厚度，4是指白线比例)
	//	m_hResult.mapFeaturelists[nIndx][8] = std::make_pair("标志位", 3);
	//	m_hResult.mapFeaturelists[nIndx][2] = std::make_pair("X坐标", 0.0f);
	//	m_hResult.mapFeaturelists[nIndx][3] = std::make_pair("Y坐标", 0.0f);
	//	m_hResult.mapFeaturelists[nIndx][4] = std::make_pair("缺陷宽", (m_CropImg.nWidth - 1)*1.0f);
	//	m_hResult.mapFeaturelists[nIndx][5] = std::make_pair("缺陷高", (m_CropImg.nHeight - 1)*1.0f);
...
int CKxPlateEdgeCheck::BrokenSolve(const kxCImageBuf& SrcImg, kxCImageBuf& WhiteLineArea, kxCImageBuf& DstImgClose, int &middleindex, Ipp32f* midrecords)
{
	...
	if (nbrokennum != 0)
	{
		...
		for (int i = 0; i < nbrokennum; i++)
		{
			...
			m_hResult.mapFeaturelists[nIndx][8] = std::make_pair("标志位", 0);//第八位作为标志位(0是特指断的区域，1是线上的，2是其它区域)
			...
			m_hResult.mapFeaturelists[nIndx][9] = std::make_pair("物理宽度", brokenrecord[idx[i]].nlen* 1.95);//暂时拿第六位作为断的实际大小
			m_hResult.nCheckStatus = _BLOBERR;
		}
		delete[]idx;
	}
...
int CKxPlateEdgeCheck::WhiteLineSolve(kxCImageBuf& WhiteLineArea, int middleindex, Ipp32f* midrecords, int& averageup, int& averagedown)
{
	...
	//表白线位置比例
	{
		...
		int nIndx = (std::min)(m_hResult.nDefectCount++, (int)_MAX_DEFECT_COUNT - 1);
		//暂时拿第八位作为标志位(0是特指断的区域，1是线上的，2是其它区域，3是指极片厚度，4是指白线比例)
		m_hResult.mapFeaturelists[nIndx][8] = std::make_pair("标志位", 4);
	...
	if (statsbad.rows > 1)
	{
		...
		for (int i = 0; i < finalnum; i++)
		{
			...
			if (nIndx < _MAX_DEFECT_COUNT - 1)
			{
				...
				m_hResult.mapFeaturelists[nIndx][8] = std::make_pair("标志位", 1);//暂时拿第八位作为标志位(0是特指断的区域，1是线上的，2是其它区域)
...
int CKxPlateEdgeCheck::WhiteLineSolve2(const kxCImageBuf& SrcImg, kxCImageBuf& whitelinearea, std::vector <RectBlob>& vec_blob, int middleidx, KxCallStatus& hCall)
{
	...
	if (statsbad.rows > 1)
	{
		...
		for (int i = 0; i < listheightsortidx.rows - 1; i++)
		{
			...
			int area = statsbad.at<int>(listheightsortidx.at<int>(i, 0), cv::CC_STAT_AREA);
			float rate = 0;
			if (height < _GLITCH_ARCH_SMALLEST)//根据情况使用哪个，因为getrate2耗时
			{
