...
from kxpyqtgraph.kxparameterTree.KxParameter import kxregisterParameterType
...
"""
    Date:   2020.03.02
    Author: HYH
    Desc:   这里是将paramterType复制过来，将基类改变，目的是实现Paramter新添加的功能
"""
    ...
    **Registered Types:**
    ...
    def makeWidget(self):
        ...
        if t in ('int', 'float'):
            ...
            w.setMaximumHeight(30)#HYH 2020.03.07
        elif t == 'bool':
            w = QtGui.QCheckBox()
            ...
            w.setMaximumHeight(30)#HYH 2021.03.06
            w.setStyleSheet("font: 10pt \"Arial\";")
...
class SimpleParameter(KxParameter):
    ...
    def colorValue(self):
        return fn.mkColor(KxParameter.value(self))

    def saveColorState(self, *args, **kwds):
        state = KxParameter.saveState(self, *args, **kwds)
...
kxregisterParameterType('int', SimpleParameter, override=True)
kxregisterParameterType('float', SimpleParameter, override=True)
kxregisterParameterType('bool', SimpleParameter, override=True)
kxregisterParameterType('str', SimpleParameter, override=True)
kxregisterParameterType('color', SimpleParameter, override=True)
kxregisterParameterType('colormap', SimpleParameter, override=True)
...
class GroupParameter(KxParameter):
    """
    Group parameters are used mainly as a generic parent item that holds (and groups!) a set
    of child parameters.
...
kxregisterParameterType('group', GroupParameter, override=True)
...
class ListParameter(KxParameter):
...
kxregisterParameterType('list', ListParameter, override=True)
...
class ActionParameter(KxParameter):
...
kxregisterParameterType('action', ActionParameter, override=True)
...
class TextParameter(KxParameter):
    """Editable string; displayed as large text box in the tree."""
    itemClass = TextParameterItem


kxregisterParameterType('text', TextParameter, override=True)
